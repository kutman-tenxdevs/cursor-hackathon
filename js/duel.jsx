...
function DuelScreen({ back, me, onEarn, toast, device }) {
  ...
  function flip() {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => { setPhase("play"); addMsg({ type: "sys", text: w === "me" ? "You won the toss — you issue first" : `${opp.name.split(" ")[0]} won the toss — they go first` }); }, 850);
    }, 2400);
  }
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      else { addMsg({ type: "proof", who: "opp", kind: ["photo","link","text"][Math.floor(Math.random()*3)], note: "Done — proof attached" }); const pts = Math.round(70 + Math.random()*60); addMsg({ type: "score", who: "opp", pts }); setScore((s) => ({ ...s, opp: s.opp + pts })); advance(); }
    }, oppIssues ? 1600 : 2000);
    return () => clearTimeout(t);
  }, [phase, step, stage, issuer]);

  function advance() { setTimeout(() => { if (step >= 5) setPhase("done"); else { setStep((s) => s + 1); setStage("issue"); setProof("photo"); setFilled(false); setDraft(""); } }, 450); }
  function sendChallenge() { if (!draft.trim()) return; addMsg({ type: "challenge", who: "me", text: draft.trim() }); setDraft(""); setStage("respond"); }
  function sendProof() { addMsg({ type: "proof", who: "me", kind: proof, note: "Submitted" }); const pts = Math.round(80 + Math.random()*55); addMsg({ type: "score", who: "me", pts }); setScore((s) => ({ ...s, me: s.me + pts })); onEarn(pts); setFilled(false); advance(); }
  ...
  if (phase === "lobby") return <>
    ...
    <div className="screen fade min-fill"><div className="pad col">
      <div className="eyebrow mb8">Six challenges · three each</div>
      <h1 className="display">Pick your battle</h1>
      <p className="body mt12">Issue challenges, complete theirs, outscore your rival across three rounds.</p>
      <div className="col g12 mt24" style={{ gap: 12 }}>
        <button onClick={() => findMatch("random")} className="card card-tap" style={{ padding: 16, display: "flex", gap: 14, alignItems: "center", textAlign: "left" }}>
          ...
          <div className="grow"><div className="h2">Random match</div><div className="small mt4">Paired with a rival near your rank.</div></div>
        ...
        <button onClick={() => findMatch("friend")} className="card card-tap" style={{ padding: 16, display: "flex", gap: 14, alignItems: "center", textAlign: "left" }}>
          ...
          <div className="grow"><div className="h2">Invite a friend</div><div className="small mt4">Share a link and duel someone you know.</div></div>
          <IconChevR size={20} style={{ color: "var(--text-3)" }} />
        </button>
      </div>
      <div className="card card-pad mt24">
        <div className="eyebrow mb16">Recent duels</div>
        <div className="col g14" style={{ gap: 14 }}>
          {[["Yui T.","W",412,388],["Dario S.","L",290,355]].map(([n,r,a,b]) =>
            <div key={n} className="row between">
              ...
              <div className="row g10" style={{ gap: 10 }}><span className="mono small">{a}–{b}</span><span className="small" style={{ color: r==="W"?"var(--easy)":"var(--hard)", fontWeight: 600 }}>{r==="W"?"Won":"Lost"}</span></div>
            </div>)}
        </div>
      </div>
    </div></div>
  ...
  if (phase === "searching") return <>
    ...
    <div className="screen fade col center" style={{ padding: "70px 18px", gap: 22 }}>
      ...
      <div className="tac"><div className="h2">{mode === "random" ? "Finding a rival…" : "Waiting for your friend…"}</div><div className="small mt4 mono">{mode === "random" ? "matching · Gold tier" : "invite link copied"}</div></div>
      <button className="btn btn-ghost sm" onClick={() => setPhase("lobby")}>Cancel</button>
    </div>
  </>;
  ...
  if (phase === "coinflip") return <>
    ...
    <div className="screen fade col center" style={{ padding: "40px 18px", gap: 26 }}>
      <div className="row center" style={{ gap: 22 }}>
        <div className="col center g8" style={{ gap: 8 }}><Avatar initials={me.initials} size={52} color={me.color} ring /><span className="small">You</span></div>
      ...
      <div className={"coin" + (flipping ? " flipping" : "") + (first ? " landed-" + first : "")}>
        <div className="coin-face coin-you"><IconUser size={28} /><span>YOU</span></div>
        <div className="coin-face coin-opp"><IconUsers size={28} /><span>{opp.name.split(" ")[0].toUpperCase()}</span></div>
      </div>
      {!first
        ? <div className="tac"><div className="eyebrow mb12">{flipping ? "Flipping…" : "Who goes first?"}</div>{!flipping && <button className="btn btn-primary lg" onClick={flip} style={{ minWidth: 200 }}><IconDice size={18} />Flip the coin</button>}</div>
        : <div className="tac pop"><div className="h1">{first === "me" ? "You're up first" : `${opp.name.split(" ")[0]} goes first`}</div><div className="small mt4">Starting the duel…</div></div>}
    </div>
  </>;
  ...
  if (phase === "done") {
    ...
    return <>
      <DuelHead back={back} title="Result" />
      <div className="screen fade min-fill"><div className="pad col">
        <div className="tac mt8">
          ...
          <div className="eyebrow mt16">{won ? "Victory" : "Defeat"}</div>
          <h1 className="display mt8">{won ? "You won the duel" : `${opp.name.split(" ")[0]} won`}</h1>
        </div>
        <div className="card card-pad row mt20" style={{ alignItems: "center" }}>
          <div className="grow col center tac" style={{ gap: 8, alignItems: "center" }}><Avatar initials={me.initials} size={38} color={me.color} /><Stat label="You" value={score.me} color="var(--accent-text)" /></div>
        ...
        <div className="card card-pad row between mt12"><span className="small" style={{ color: "var(--text)" }}>{won ? "Match winnings" : "Consolation"}</span><Pts v={won ? 220 : 60} sign size={17} color={won ? "var(--accent-text)" : "var(--text-2)"} /></div>
        <div className="row g10 mt24" style={{ gap: 10 }}>
          <button className="btn btn-ghost lg grow" onClick={() => { setPhase("lobby"); setStep(0); setStage("issue"); setMsgs([]); setScore({me:0,opp:0}); setFirst(null); }}><IconRefresh size={18} />Rematch</button>
  ...
  return <>
    ...
    <div className="pad" style={{ paddingTop: 0, paddingBottom: 12 }}>
      <div className="row between mb8"><span className="eyebrow">Round {Math.floor(step/2)+1} of 3</span><span className="small mono">{6 - msgs.filter(m=>m.type==='score').length} left</span></div>
    ...
    <div className="screen" ref={threadRef}>
      <div className="pad chat" style={{ paddingTop: 4 }}>
        ...
        {myTurnIssue && <div className="bubble sys">Your turn to issue a challenge</div>}
        {myTurnRespond && <div className="bubble sys">Complete their challenge & submit proof</div>}
      </div>
    </div>
    {myTurnIssue && <div className="input-zone">
      ...
      <div className="row g8" style={{ gap: 8 }}>
        <input className="field grow" value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Issue a challenge…" onKeyDown={(e)=> e.key==='Enter' && sendChallenge()} />
    ...
    {myTurnRespond && <div className="input-zone">
      <ProofPicker value={proof} onChange={setProof} allow={["photo","link","text"]} />
      {!filled
        ? <button className="btn btn-ghost block mt12" onClick={() => setFilled(true)}>{proof === "photo" ? <><IconImage size={18}/>Attach photo</> : proof === "link" ? <><IconLink size={18}/>Paste a link</> : <><IconChat size={18}/>Write your answer</>}</button>
        : <button className="btn btn-primary block mt12" onClick={sendProof}><IconCheck size={18} />Submit proof</button>}
    </div>}
  </>;
}

function DuelHead({ back, title = "Duel" }) {
  return <div className="flowhead">
    <button className="icon-btn" onClick={back}><IconArrowL size={18} /></button>
    <div className="grow"><div className="fh-title row g8" style={{ gap: 8 }}>{title} <span className="tag" style={{ height: 19 }}>1v1</span></div></div>
  </div>;
}
...
function DuelMsg({ m, me, opp }) {
  ...
  if (m.type === "challenge") return <div className="row g8" style={{ gap: 8, flexDirection: mine ? "row-reverse" : "row", alignItems: "flex-start" }}>
    ...
    <div className="card card-pad" style={{ maxWidth: "80%", padding: 12, borderColor: mine ? "var(--accent-line)" : "var(--border)", background: mine ? "var(--accent-soft)" : "var(--surface)" }}>
      <div className="eyebrow mb8" style={{ color: mine ? "var(--accent-text)" : "var(--text-3)" }}>{mine ? "You issued" : who.name.split(" ")[0] + " issued"}</div>
  ...
  if (m.type === "proof") {
    ...
    return <div className="row g8" style={{ gap: 8, flexDirection: mine ? "row-reverse" : "row", alignItems: "flex-start" }}>
      ...
      <div style={{ maxWidth: "80%" }}>
        {m.kind === "photo" && <Placeholder label="proof photo" h={110} style={{ width: 168, marginBottom: 6 }} />}
  ...
  if (m.type === "score") return <div className="bubble sys" style={{ color: mine ? "var(--accent-text)" : "var(--text-2)" }}>{mine ? "You" : who.name.split(" ")[0]} scored +{m.pts} pts</div>;
  return null;
}
